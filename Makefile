...
run-x11:
	...
	@$(MAKE) --no-print-directory justrun-x11

justrun-x11:
	@img=$$(python3 ./scripts/lab-run.py x11 --stage-only --skip-build --boot-timeout 120 --command-timeout 600); \
	$(MAKE) --no-print-directory GRAPHIC=y INPUT=y NET=n HOSTFWD=n APP_FEATURES=qemu,lab DISK_IMG=$$img justrun
...
lab-repeat-x11:
	python3 ./scripts/lab-run.py x11 --boot-timeout 120 --command-timeout 600 $(LAB_RAW_FLAG) --repeat 2
